...
function saveMood($date, $mood)
{
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $sql = "UPDATE diaries SET mood = :mood WHERE created_at = :date";
        ...
        $content = null;
        $image = null;
        $sql = "INSERT INTO diaries (title, content, created_at, image, mood) 
         VALUES (:title, :content, :created_at, :image, :mood)";
        $stmt = $conn->prepare($sql);
